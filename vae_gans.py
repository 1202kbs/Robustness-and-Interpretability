...
class Model:
    ...
    def reconstruct(self, sess, dataset, batch_size=100):
        ...
        n_itrs = int(math.ceil(len(dataset[0]) / batch_size))
        res = []
    ...
    def generate(self, sess, N, batch_size=100):
        ...
        n_itrs = int(math.ceil(N / batch_size))
        res = []
            ...
            with tf.name_scope('Losses'):
                ...
                with tf.name_scope('KL_Loss'):
                    kl_loss = -0.5 * tf.reduce_sum(
                        1 + self.z_sigma_log_sq - tf.square(self.z_mean) - tf.exp(self.z_sigma_log_sq), axis=1)
                ...
                with tf.name_scope('Dec_Loss'):
                    self.dec_loss = tf.reduce_mean(
                        self.lmda * reconst_loss - tf.log(d_dec + 1e-8) - tf.log(d_fake + 1e-8))

                with tf.name_scope('Disc_loss'):
                    self.disc_loss = -tf.reduce_mean(
                        tf.log(d_real + 1e-8) + tf.log(1 - d_dec + 1e-8) + tf.log(1 - d_fake + 1e-8))

                with tf.variable_scope('Enc_Train'):
                    with tf.control_dependencies(
                            tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope=self.name + '/Encoder')):
                        enc_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name + '/Encoder')
                        self.enc_train = tf.train.AdamOptimizer(self.learning_rate, self.beta1, self.beta2).minimize(
                            self.enc_loss, var_list=enc_vars)

                with tf.variable_scope('Dec_Train'):
                    with tf.control_dependencies(
                            tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope=self.name + '/Decoder')):
                        dec_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name + '/Decoder')
                        self.dec_train = tf.train.AdamOptimizer(self.learning_rate, self.beta1, self.beta2).minimize(
                            self.dec_loss, var_list=dec_vars)
                ...
                with tf.variable_scope('Disc_Train'):
                    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope=self.name + '/Disc')):
                        ...
                        self.disc_train = tf.train.AdamOptimizer(self.learning_rate, self.beta1, self.beta2).minimize(
...
class MNIST_VAE_GAN(Model):
    def __init__(self, logdir, lmda=1.0, zdim=10, learning_rate=5e-4, beta1=0.5, beta2=0.9, name='MNIST_VAE_GAN'):
        ...
        dec_params2 = {'training': self.training, 'kernel_size': [4, 4], 'activation': tf.nn.relu,
                       'use_batchnorm': True}
        ...
        self.dec_layers = [RegDense(units=2 * 2 * 256, name='dense1', **dec_params1),
                           RegConv2DTranspose(filters=256, strides=(2, 2), padding='SAME', name='deconv2',
                                              **dec_params2),
                           RegConv2DTranspose(filters=128, strides=(1, 1), padding='VALID', name='deconv3',
                                              **dec_params2),
                           RegConv2DTranspose(filters=32, strides=(2, 2), padding='SAME', name='deconv4',
                                              **dec_params2),
...
class CIFAR_VAE_GAN(Model):
    def __init__(self, logdir, lmda=1e-3, zdim=128, learning_rate=2e-4, beta1=0.0, beta2=0.9, name='CIFAR_VAE_GAN'):
        ...
        self.dec_layers = [RegDense(units=4 * 4 * 512, name='dense1', **dec_params1),
                           RegConv2DTranspose(filters=256, strides=(2, 2), padding='SAME', name='deconv2',
                                              **dec_params2),
                           RegConv2DTranspose(filters=128, strides=(2, 2), padding='SAME', name='deconv3',
                                              **dec_params2),
                           RegConv2DTranspose(filters=64, strides=(2, 2), padding='SAME', name='deconv4',
                                              **dec_params2),
