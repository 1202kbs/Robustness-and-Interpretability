...
class RegLayer(object):
    ...
    def __call__(self, inputs):
        ...
        with tf.variable_scope(self.name):
            ...
            if self.use_batchnorm: outputs = tf.layers.batch_normalization(inputs=outputs, training=self.training,
                                                                           name='BatchNorm')
            if self.droprate: outputs = tf.layers.dropout(inputs=outputs, rate=self.droprate, training=self.training,
                                                          name='Dropout')
...
class RegDense(RegLayer):
    def __init__(self, training, units, activation=None, use_batchnorm=False, droprate=None, snbeta=None, l2rate=None,
                 name=None):
    ...
    def _activation(self, inputs):
        ...
        self.kernel = tf.get_variable(name='kernel', shape=[inputs.shape[-1], self.units],
...
class RegConv2D(RegLayer):
    def __init__(self, training, filters, kernel_size, strides=(1, 1), padding='VALID', activation=None,
                 use_batchnorm=False, droprate=None, snbeta=None, l2rate=None, name=None):
    ...
    def _activation(self, inputs):
        ...
        self.kernel = tf.get_variable(name='kernel',
                                      shape=[self.kernel_size[0], self.kernel_size[1], inputs.shape[-1], self.filters],
                                      initializer=tf.glorot_uniform_initializer())
        ...
        return self.activation(
            tf.nn.conv2d(input=inputs, filter=self.kernel, strides=[1, self.strides[0], self.strides[1], 1],
...
class RegConv2DTranspose(RegLayer):
    ...
    def _activation(self, inputs):
        ...
        self.kernel = tf.get_variable(name='kernel',
                                      shape=[self.kernel_size[0], self.kernel_size[1], self.filters, inputs.shape[-1]],
                                      initializer=tf.glorot_uniform_initializer())
        self.bias = tf.get_variable(name='bias', shape=[self.filters], initializer=tf.constant_initializer(0.0))
        ...
        if self.padding == 'SAME':
            output_shape = [tf.shape(inputs)[0], inputs_shape[1] * self.strides[0], inputs_shape[2] * self.strides[1],
                            self.filters]
        else:
        ...
        return self.activation(tf.nn.conv2d_transpose(value=inputs, filter=self.kernel, output_shape=output_shape,
