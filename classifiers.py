        ...
        with tf.name_scope('Losses'):
            ...
            with tf.name_scope('CELoss'):
                ...
                self.cross_entropy = tf.reduce_mean(
                    tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.logits, labels=self.Y_hot))
...
class MNIST_CNN(Base_Classifier):
    def __init__(self, logdir, n_classes=10, activation=None, use_batchnorm=False, droprate=None, snbeta=None,
                 ...
                 name='MNIST_CNN'):
        super(MNIST_CNN, self).__init__(logdir, n_classes, activation, use_batchnorm, droprate, snbeta, l2rate, attack,
                                        attack_params, optimizer, learning_rate, name)
        ...
        params2 = {'training': self.training, 'activation': self.activation, 'use_batchnorm': self.use_batchnorm,
                   'droprate': self.droprate, 'snbeta': self.snbeta, 'l2rate': self.l2rate}
    ...
    def classify(self, inputs):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('Block1'):
                outputs = tf.layers.max_pooling2d(reduce((lambda x, y: y(x)), [inputs] + self.layers[:2]), name='pool2',
                                                  **pooling_params)
...
class CIFAR_CNN(Base_Classifier):
    def __init__(self, logdir, n_classes=10, activation=None, use_batchnorm=False, droprate=None, snbeta=None,
                 l2rate=None, attack=None, attack_params=None, optimizer=tf.train.AdamOptimizer, learning_rate=1e-3,
                 name='CIFAR_CNN'):
        super(CIFAR_CNN, self).__init__(logdir, n_classes, activation, use_batchnorm, droprate, snbeta, l2rate, attack,
                                        attack_params, optimizer, learning_rate, name)
    ...
    def classify(self, inputs):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('Block2'):
                outputs = tf.layers.max_pooling2d(reduce((lambda x, y: y(x)), [outputs] + self.layers[2:4]),
                                                  name='pool4', **pooling_params)
