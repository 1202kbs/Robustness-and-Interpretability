...
def SN_Conv(inputs, kernel, strides, padding='VALID', beta=1.0, Ip=1, reuse=False, name='SN_Conv'):
    ...
    def power_iteration_conv(u, w, Ip):
        ...
        for _ in range(Ip):
            ...
            u_ = tf.nn.l2_normalize(
                tf.nn.conv2d_transpose(v_, w, [1, u_width, u_width, u_depth], strides=[1, strides[0], strides[1], 1],
                                       padding=padding))
    ...
    with tf.variable_scope(name) as scope:
        ...
        u = tf.get_variable('u', shape=[1, u_width, u_width, u_depth], initializer=tf.truncated_normal_initializer(),
                            trainable=False)
